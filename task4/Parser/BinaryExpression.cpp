...
bool BinaryExpression::isEqual(const Expression *expr) const
{
    return typeid(*this) == typeid(*expr)
            && left->isEqual(static_cast<const BinaryExpression*>(expr)->left.get())
            && right->isEqual(static_cast<const BinaryExpression*>(expr)->right.get());
}

SubstState BinaryExpression::isFreeToSubstitute(const std::string &varName, const std::vector<std::string> &freeVariables) const
{
    SubstState res(left->isFreeToSubstitute(varName, freeVariables));
    res = right->isFreeToSubstitute(varName, freeVariables);
    return res;
}

bool BinaryExpression::isSubstitute(const Expression *expr) const
{
    if (typeid(*this) == typeid(*expr) && false)
    {
        std::cerr << toString() << "\n";
        std::cerr << "BE " <<
                left->isSubstitute(static_cast<const BinaryExpression*>(expr)->left.get())
                        << " " << right->isSubstitute(static_cast<const BinaryExpression*>(expr)->right.get()) << "\n";
    }
    return typeid(*this) == typeid(*expr)
            && left->isSubstitute(static_cast<const BinaryExpression*>(expr)->left.get())
            && right->isSubstitute(static_cast<const BinaryExpression*>(expr)->right.get());

...
//    int num = 0;
    ...
    {
        std::shared_ptr<const Expression> myParser(Parser::parseString(block->toString()));
        if (proofCheck.isAxiom(myParser.get()) == -1
                && proofCheck.getModusPones(myParser.get()).first == (size_t) -1)
        {
            std::cerr << "PANIC WRONG STEP " << block->toString() << "\n";
            exit(-1);
        }
//        std::cerr << ++num << " " << proofCheck.isAxiom(myParser.get()) << " " << proofCheck.getModusPones(myParser.get()).first << "\n";
        proofCheck.addExpression(myParser);
    }
    for (auto block : blocks.begin()->second)
    {
        cout << block->toString() << "\n";
